SUBROUTINE DIFFCORD_MATRIX_DECOMP_HOBN   !计算差分
    USE GREENMOD
	USE CUMOD
    ...
    DO IX=1,NFXF+NFXA+NFX 
        DO IY=1,NFY   
            IF(IY.EQ.1) THEN      !奇数行
                ...
                DMET(J,J1)=C21
                DMET(J,J)=C22
            ELSE IF(IY.EQ.2) THEN      !偶数行
                J=M+(IX-1)*NFY+IY
                J1=J+2
                !DCORD(J,4)=CORD(J1,2)*C21+CORD(J,1)*C22
                DMET(J,J1)=C21
                DMET(J,J)=C22
            ELSE IF(IY.GE.3.AND.IY.LE.NFY-2) THEN
                J=M+(IX-1)*NFY+IY
                J1=J+2
                J_1=J-2
...
SUBROUTINE DIFFCORD_DECOMP_HOBN     !计计算坐标对计算平面导数
    USE GREENMOD
	USE CUMOD
